...
@router.message(Text("🔑 Авторизоваться"))
async def start_auth(message: types.Message, state: FSMContext):
    """
    Старт авторизации — спрашиваем номер договора
    """
...
async def auth_get_contract(message: types.Message, state: FSMContext):
    """
    Получаем номер договора, спрашиваем пароль статистики
    """
...
async def auth_wrong_contract(message: types.Message):
    """
    Если номер договора не соответствует формату
    """
...
async def auth_get_password(message: types.Message, state: FSMContext):
    """
    Получаем пароль, пытаемся авторизоваться в биллинге
    """
    ...
    try:
        ...
        if auth_response.get("status") == "Ok":
            ...
            await message.answer(
                ...
                f"📄 Договор: <b>{contract_title}</b>",
                reply_markup=await main_menu(chat_id)
            )
            logger.info(f"Пользователь {chat_id} авторизован (договор {contract_id})")

        else:
            await message.answer("❌ Неверный номер договора или пароль.", reply_markup=main_menu())
            logger.warning(f"Неудачная попытка авторизации: chat_id={chat_id}, contract={contract_id}")

    except Exception as e:
        await message.answer("⚠ Ошибка при авторизации. Попробуйте позже.", reply_markup=main_menu())
